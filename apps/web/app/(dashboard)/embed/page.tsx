...
interface EmbedConfigCardProps {
  ...
  editSiteId: string
  setEditSiteId: (value: string) => void
  editThemeColor: string
  setEditThemeColor: (value: string) => void
  editButtonLabel: string
  setEditButtonLabel: (value: string) => void
  editAllowDomains: string
  setEditAllowDomains: (value: string) => void
  editLocale: string
  setEditLocale: (value: string) => void
  editTimezone: string
  setEditTimezone: (value: string) => void
  editDefaultService: string
  setEditDefaultService: (value: string) => void
  editSaving: boolean
  editError: string | null
  onStartEdit: (config: EmbedConfig) => void
  onCancelEdit: () => void
  onSaveEdit: () => void
}
    ...
    <div className="border border-gray-200 rounded-lg p-4">
        ...
        <div className="mt-4 bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={onSaveEdit}
              disabled={editSaving}
              className="px-3 py-1.5 bg-indigo-600 text-white rounded-md text-sm hover:bg-indigo-700 disabled:opacity-60"
            >
      ...
      {auditLogs.length > 0 && (
        <div className="mt-4 border-t border-gray-200 pt-3">
          <div className="text-xs font-semibold text-gray-700 mb-2">Audit Log</div>
          <ul className="space-y-1 text-xs text-gray-500">
            {auditLogs.map((log) => (
              <li key={log._id}>
                {new Date(log.timestamp).toLocaleString()} • {log.action} • {log.userId}
              </li>
            ))}
          </ul>
        </div>
          ...
          <div className="space-y-4">
            {configs.map((config) => (
              <EmbedConfigCard
                ...
                isEditing={editConfigId === config._id}
                isStaff={isStaff}
                sites={sites}
                ...
                editThemeColor={editThemeColor}
                setEditThemeColor={setEditThemeColor}
                editButtonLabel={editButtonLabel}
                setEditButtonLabel={setEditButtonLabel}
                editAllowDomains={editAllowDomains}
                setEditAllowDomains={setEditAllowDomains}
                editLocale={editLocale}
                setEditLocale={setEditLocale}
                editTimezone={editTimezone}
                setEditTimezone={setEditTimezone}
                editDefaultService={editDefaultService}
                setEditDefaultService={setEditDefaultService}
